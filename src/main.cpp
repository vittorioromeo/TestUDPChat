...
enum PTFromServer : PTType
{
    Accept,
    FSMessage
};
enum PTFromClient : PTType
{
    Connect,
    Ping,
    FCMessage
...
inline sf::Packet buildPacketFromServer(TArgs&&... mArgs)
{
    return ::Impl::buildPacket<PT::FromServer>(TP, FWD(mArgs)...);
}
template <PTFromClient TP, typename... TArgs>
inline sf::Packet buildPacketFromClient(TArgs&&... mArgs)
{
    return ::Impl::buildPacket<PT::FromClient>(TP, FWD(mArgs)...);
}
...
public:
    inline void handle(PTType mType, T& mCaller, sf::Packet& mPacket)
    {
        ...
        {
            lo("PacketHandler") << "Exception during packet handling: ("
                                << mType << ")\n" << mEx.what() << "\n";
        }
        catch(...)
        {
            lo("PacketHandler") << "Unknown exception during packet handling: ("
                                << mType << ")\n";
        }
    }
...
class ClientHandler
{
    // A ClientHandler is a child object of a Server which deals with a specific
    // Client
    // It can be attached to a Client (has an accepted Client) or not (free and
...
public:
    ...
    {
        // Accepts a Client: sets attachedToClient to true and starts a new
        // thread
...
struct Client
{
    ...
    inline void send(sf::Packet mPacket)
    {
        if(socket.send(mPacket, serverIp, serverPort) != sf::Socket::Done)
            lo("Client") << "Error sending"
                         << "\n";
    }
};
...
struct Server
{
    ...
    {
        for(auto& c : clientHandlers)
            if(c->isAttachedToClient()) c->sendToClient(mPacket);
    }
};
...
int main()
{
    ...
    while(true)
    {
        ...
        if(std::getline(std::cin, input))
        {
            if(input == "exit") break;
            Packet
    clientMsg{buildPacketFromClient<PTFromClient::FCMessage>(c.uid, input)};
            c.send(clientMsg);
        }
